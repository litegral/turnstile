...
type eventStore interface {
	...
	Fail(context.Context, Event, error, time.Duration, int) (bool, error)
}
...
func NewWorker(store eventStore, handler Handler, config Config, logger *slog.Logger) (*Worker, error) {
	...
	if config.Concurrency < 1 || config.MaxAttempts < 1 {
		return nil, errors.New("outbox worker counts must be positive")
	}
...
func (w *Worker) processOne(ctx context.Context) (bool, error) {
	...
	delay, delayErr := retryDelay(event.AttemptCount+1, w.config.BaseRetryDelay, w.config.MaximumRetryDelay)
	if delayErr != nil {
		return true, delayErr
	}
	updated, failErr := w.store.Fail(ctx, event, err, delay, w.config.MaxAttempts)
	if failErr != nil {
		return true, failErr
	}
	if !updated {
